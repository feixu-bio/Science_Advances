...
C{1,1} = '4_cell_EMS_zygoticgene.txt';
C{2,1} = '8_cell_E_zygoticgene.txt';
C{3,1} = '8_cell_MS_zygoticgene.txt';
C{4,1} = '16_cell_new_Ex_zygoticgene.txt';
C{5,1} = '16_cell_new_MSx_zygoticgene.txt';


T1 = readtable("Annotation_Gene.txt");
T3 = readtable("EMR-1_DamID-med1p-bound-bed-1707061998.txt");

for dataset = 1:length(C)
    T2 = readtable(C{dataset,1});
    AnoGenArray = [T1.Var18];
    AnoWBGenArray = [T1.Var10];
    for i = 1:height(T2)
        Gene = string(T2.zygGene(i));
        index1 = find(ismember(AnoGenArray,Gene));
        index2 = find(ismember(AnoWBGenArray,Gene));
        if ~isempty(index1)
        ...
        elseif ~isempty(index2)
            T2.Chr(i) = append('chr', T1.x__genebuild_versionWS285(index2));
            T2.start(i) = T1.Var4(index2);
            T2.end(i) = T1.Var5(index2);
            T2.mid(i) = mean(T1.Var4(index2),T1.Var5(index2));
        end
    end
    ...
    writetable(T2,['DamID_' C{dataset,1}],'Delimiter',',');
end
